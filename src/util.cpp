...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "unitcoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No unitcoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override unitcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "unitcoind.pid"));
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    LogPrintf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("UnitCoin"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING);
                }
            }
        }
        if (fDebug) {
