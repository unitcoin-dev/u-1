...
#include "unitbridge.moc"

UnitBridge::UnitBridge(UnitGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void UnitBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void UnitBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void UnitBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void UnitBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void UnitBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void UnitBridge::populateOptions()
{
...
// Transactions
bool UnitBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void UnitBridge::clearRecipients()
{
    recipients.clear();
}

bool UnitBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 UNIT, 1 Unit
        switch(rcp.txnTypeInd)
        {
            case TXT_UNIT_TO_UNIT:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::UNIT, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_UNIT_TO_ANON:
                formatted.append(tr("<b>%1</b> to UNIT %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::UNIT, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> UNIT, ring size %2 to UNIT %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::UNIT, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_UNIT:
                formatted.append(tr("<b>%1</b> UNIT, ring size %2 to UNIT %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::UNIT, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::UNIT, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsUnitBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your unit balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::UNIT, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void UnitBridge::openCoinControl()
{
...
void UnitBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void UnitBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap UnitBridge::listAnonOutputs()
{
...
void UnitBridge::populateTransactionTable()
{
...
void UnitBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void UnitBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString UnitBridge::transactionDetails(QString txid)
{
...
// Addresses
void UnitBridge::populateAddressTable()
{
...
void UnitBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void UnitBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString UnitBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString UnitBridge::lastAddressError()
{
    QString sError;
...
QString UnitBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void UnitBridge::updateAddressLabel(QString address, QString label)
{
...
bool UnitBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool UnitBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void UnitBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void UnitBridge::appendMessage(int row)
{
...
void UnitBridge::populateMessageTable()
{
...
void UnitBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool UnitBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool UnitBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString UnitBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString UnitBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool UnitBridge::setPubKey(QString address, QString pubkey)
{
...
bool UnitBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString UnitBridge::createGroupChat(QString label)
{
...
QString UnitBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList UnitBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void UnitBridge::connectSignals()
{
...
QString UnitBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap UnitBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap UnitBridge::listLatestBlocks()
{
...
QVariantMap UnitBridge::findBlock(QString searchID)
{
...
QVariantMap UnitBridge::blockDetails(QString blkHash)
{
...
QVariantMap UnitBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap UnitBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap UnitBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap UnitBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap UnitBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap UnitBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap UnitBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap UnitBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap UnitBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Unitcoin prefix.");
            return result;
        }
...
QVariantMap UnitBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap UnitBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap UnitBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
