...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return UnitGUI::tr("Received with");
    case RecvFromOther:
        return UnitGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return UnitGUI::tr("Sent to");
    case SendToSelf:
        return UnitGUI::tr("Payment to yourself");
    case Generated:
        return UnitGUI::tr("Mined");
    case RecvUnit:
        return UnitGUI::tr("Received unit");
    case SendUnit:
        return UnitGUI::tr("Sent unit");
    case Other:
        return UnitGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvUnit:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendUnit:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredUNIT, nCredUnit;
    wtx.GetCredit(nCredUNIT, nCredUnit, true);
    int64_t nCredit = nCredUNIT + nCredUnit;
    ...
    {
        if (nNet > 0 && nCredUnit > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvUnit, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendUnit, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvUnit;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv unit
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendUnit;
                    sub.address = CBitcoinAddress(ckidD).ToString();
