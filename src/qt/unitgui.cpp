// Copyright (c) 2014 The UnitCoin developers
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("UnitClient") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/unit"));
    setWindowIcon(QIcon(":icons/unit"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/unit/index.html");
#else
    QFile html("/opt/unit/index.html");
#endif
...
void UnitGUI::pageLoaded(bool ok)
{
...
void UnitGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void UnitGUI::urlClicked(const QUrl & link)
{
...
void UnitGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/unit"), tr("&About UnitCoin"), this);
    ...
    toggleHideAction = new QAction(QIcon(":/icons/unit"), tr("&Show / Hide"), this);
...
void UnitGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("UnitClient") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/unit_testnet"));
            setWindowIcon(QIcon(":icons/unit_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/unit_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("UnitClient client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/unit_testnet"));
...
void UnitGUI::setWalletModel(WalletModel *walletModel)
{
...
void UnitGUI::setMessageModel(MessageModel *messageModel)
{
...
void UnitGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("UnitClient client"));
    trayIcon->setIcon(QIcon(":/icons/unit"));
...
#ifndef Q_OS_MAC
void UnitGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void UnitGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to UnitCoin network", "", count));
}

void UnitGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void UnitGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void UnitGUI::changeEvent(QEvent *e)
{
...
void UnitGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void UnitGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::UNIT, nFeeRequired));
...
void UnitGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void UnitGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void UnitGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void UnitGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void UnitGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void UnitGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
void UnitGUI::handleURI(QString strURI)
{
...
void UnitGUI::setEncryptionStatus(int status)
{
...
void UnitGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void UnitGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void UnitGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void UnitGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void UnitGUI::updateStakingIcon()
{
