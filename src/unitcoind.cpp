...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            // First part of help message is specific to unitcoind / RPC client
            std::string strUsage = _("UnitCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  unitcoind [options]                     " + "\n" +
                  "  unitcoind [options] <command> [params]  " + _("Send command to -server or unitcoind") + "\n" +
                  "  unitcoind [options] help                " + _("List commands") + "\n" +
                  "  unitcoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "unitcoin:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect unitcoind signal handlers
    noui_connect();
