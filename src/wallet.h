...
public:
    ...
    int64_t GetUnitDebit(const CTxIn& txin) const;
    int64_t GetUnitCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nUNIT, int64_t& nUnit) const
    {
        nUNIT = 0;
        nUnit = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nUnit += GetUnitCredit(txout);
            } else
                nUNIT += GetCredit(txout);
            if (!MoneyRange(nUNIT)
                || !MoneyRange(nUnit))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableUnitCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableUnitCreditCached;
    
    mutable int64_t nCredUNITCached;
    mutable int64_t nCredUnitCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableUnitCreditCached = false;
    ...
    bool GetCredit(int64_t& nCredUNIT, int64_t& nCredUnit, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredUNIT = nCredUnit = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredUNITCached = 0;
            nCredUnitCached = 0;
            pwallet->GetCredit(*this, nCredUNITCached, nCredUnitCached);
            fCreditSplitCached = true;
        };
        
        nCredUNIT = nCredUNITCached;
        nCredUnit = nCredUnitCached;
        return true;
    }
    ...
    int64_t GetAvailableUnitCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableUnitCreditCached)
            return nAvailableUnitCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableUnitCredit() : value out of range");
            };
        };
        
        nAvailableUnitCreditCached = nCredit;
        fAvailableUnitCreditCached = true;
        return nCredit;
    };
